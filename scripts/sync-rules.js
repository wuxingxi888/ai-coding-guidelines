...
const IDE_ADAPTERS = [
    {
        ...
        detect: (env) => {
            const hasCursorKeys = Object.keys(env).some((key) =>
            ...
            const isTerminalMatch = [
                ...
                (val) =>
                    val &&
                    (val.toLowerCase().includes("cursor") ||
                        val === "com.todesktop"),
            );
    ...
    },
    {
        ...
        detect: (env) => {
            return [
                ...
                (val) =>
                    val &&
                    (val.toLowerCase().includes("windsurf") ||
                        val === "com.exafunction.windsurf"),
            );
        },
    },
    {
...
class SyncEngine {
    ...
    detectEnvironment() {
        return (
            this.adapters.find((adapter) => adapter.detect(this.env)) || null
        );
    }
    ...
    executeSync(adapter) {
        ...
        // 1. 获取并清理所有目标目录
        const targetDirs = [
            ...new Set(adapter.mappings.map((m) => path.join(ROOT_DIR, m.to))),
        ];
        targetDirs.forEach((targetPath) => {
            if (fs.existsSync(targetPath)) {
                fs.rmSync(targetPath, { recursive: true, force: true });
                console.log(
                    `  🧹 已清理旧目录: ${path.relative(ROOT_DIR, targetPath)}`,
                );
            }
            ...
            const walk = (currentSrc, currentDest) => {
                ...
                items.forEach((item) => {
                    ...
                    if (stat.isDirectory()) {
                        ...
                        // 如果是 .md 文件，执行转换并拷贝
                        const targetName =
                            path.basename(item, ".md") + mapping.ext;
                        ...
                        try {
                            ...
                            console.log(
                                `  ✅ 已同步: ${relSrc} -> ${mapping.ext}`,
                            );
                            syncCount++;
                        } catch (err) {
                            console.error(
                                `  ❌ 同步失败 ${item}: ${err.message}`,
                            );
                        }
                    }
                });
